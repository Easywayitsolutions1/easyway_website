...
import { fetchSearchCompany, ICompanySerachView } from './SearchResultController';
...
const SearchResultView = () => {
  ...
  const [CompanySearchList, setCompanySearchList] = useState<ICompanySerachView[]>([]);
  const [loading, setLoading] = useState(false);

  const searchLocation = useLocation();
  const { searchTerm } = searchLocation.state || {};


  useEffect(() => {

    fetchSearchCompany(setCompanySearchList, searchTerm,setLoading);
  ...
  const handelRefresh = async () => {
    await fetchSearchCompany(setCompanySearchList, searchTerm,setLoading);
  };
  return (
        ...
        <div className='right-side-scroll'>
          ...
          <div className="row">
            <div className="col-md-12">
              <p className="text-center" style={{ color: "#999" }}>
                <Link className='navigation-hierarchy' to={routes.leftView}>Home</Link> {">"} {searchTerm}
              </p>
            </div>
          </div>
          <div className="chatBox">
            {loading ? (
              ...
              <div className="row">
                {CompanySearchList && CompanySearchList.length > 0 ? (
                  CompanySearchList.map((company, index) => (
                    <div
                    ...
                    >
                      <div className="card">
                        <div className="card-body" style={{ padding: "0px" }}>
                          <div className="search-company-img" style={{
                  ...
                  <p className="text-center text-muted">No Search Result Found</p>
                )}
              </div>
            )}
          </div>
